...
class MultiEthnicPopulationDataset(Dataset):
    ...
    """
    def __init__(self, features_df, population_df_2006, population_df_2016=None, 
        ...
        Args:
            ...
            population_df_2006 (pd.DataFrame): DataFrame containing 2006 multi-ethnic population data
            population_df_2016 (pd.DataFrame, optional): DataFrame containing 2016 multi-ethnic population data
        ...
        """
        # Merge features with 2006 population data on DAUID
        self.data = pd.merge(features_df, population_df_2006, on='DAUID', how='inner')
        
        # Store ethnicities columns (all except DAUID)
        self.ethnicity_columns = [col for col in population_df_2006.columns if col != 'DAUID']
        ...
        # If 2016 population data is provided (for training), merge it as well
        if population_df_2016 is not None:
            # Use suffixes to distinguish between 2006 and 2016 population columns
            self.data = pd.merge(self.data, population_df_2016, on='DAUID', 
                               how='inner', suffixes=('_2006', '_2016'))
        ...
        # Extract 2006 population vectors
        self.population_2006 = self.data[[f"{col}_2006" if population_df_2016 is not None else col 
                                        for col in self.ethnicity_columns]].values
        
        # Extract 2016 population vectors if available
        if population_df_2016 is not None:
            self.population_2016 = self.data[[f"{col}_2016" for col in self.ethnicity_columns]].values
        else:
    ...
    def __getitem__(self, idx):
        ...
        pop_2006 = self.population_2006[idx]
        ...
        # If training (2016 data available), return features and target
        if self.population_2016 is not None:
            return {
                'dauid': dauid,
                'features': torch.tensor(features, dtype=torch.float32),
                'population_2006': torch.tensor(pop_2006, dtype=torch.float32),
                'coordinates': torch.tensor(coords, dtype=torch.float32),
                'target': torch.tensor(self.population_2016[idx], dtype=torch.float32)
            }
...
class MultiEthnicDataHandler:
    ...
    def load_data(self):
        ...
        files = {
            'train_features': os.path.join(self.data_dir, '2006_train_features_all.csv'),
            'train_pop_2006': os.path.join(self.data_dir, '2006_train_population_all.csv'),
            'train_pop_2016': os.path.join(self.data_dir, '2016_train_population_all.csv'),
            'val_features': os.path.join(self.data_dir, '2006_val_features_all.csv'),
            'val_pop_2006': os.path.join(self.data_dir, '2006_val_population_all.csv'),
            'val_pop_2016': os.path.join(self.data_dir, '2016_val_population_all.csv'),
            'test_features': os.path.join(self.data_dir, '2006_test_features_all.csv'),
            'test_pop_2006': os.path.join(self.data_dir, '2006_test_population_all.csv'),
        ...
        for key, file_path in files.items():
            if os.path.exists(file_path):
                ...
                if 'test_pop_2016' in key:  # This might be intentionally missing for final testing
    ...
    def create_dataloaders(self, data, batch_size=32):
        ...
        val_dataset = None
        if 'val_features' in data and 'val_pop_2006' in data and 'val_pop_2016' in data:
        ...
        test_dataset = None
        if 'test_features' in data and 'test_pop_2006' in data:
            # For test dataset, we may or may not have 2016 population data
            test_pop_2016 = data.get('test_pop_2016')
