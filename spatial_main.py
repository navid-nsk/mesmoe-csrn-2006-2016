...
def main(args):
    ...
    logger.info(f"Population vectors dimension: {sample_batch['population_2006'].shape[1]}")
    ...
    try:
        ...
        if test_loader and any(f"{k}_2016" in data for k in ['test_pop', 'test_pop_2016']):
        ...
        # Generate predictions if test data is available but without 2016 population
        if test_loader and not any(f"{k}_2016" in data for k in ['test_pop', 'test_pop_2016']):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hidden_dim_1_base", type=int, default=20,
                        help="Base size for first hidden dimension (optimized)")
    parser.add_argument("--hidden_dim_2_base", type=int, default=10,
                        help="Base size for second hidden dimension (optimized)")
    parser.add_argument("--hidden_dim_3_base", type=int, default=4,
                        help="Base size for third hidden dimension (optimized)")
    parser.add_argument("--coord_embed_base", type=int, default=2,
                        help="Base size for coordinate embedding dimension (optimized)")
    parser.add_argument("--dropout_rate", type=float, default=0.23754701764520592,
                        help="Dropout rate (optimized)")
    parser.add_argument("--colonization_threshold", type=float, default=0.08136253205020105,
                        help="Threshold below which population is considered zero (optimized)")
    parser.add_argument("--colonization_expert_weight", type=float, default=0.7769479467834559,
                        help="Base weight for colonization expert influence (optimized)")
    parser.add_argument("--colonization_weight", type=float, default=8.05266659034115,
                        help="Weight multiplier for colonization loss (optimized)")
    parser.add_argument("--jump_threshold", type=float, default=185.56933062720324,
                        help="Threshold for identifying potential jump cases (optimized)")
    parser.add_argument("--jump_expert_weight", type=float, default=0.5681139558702257,
                        help="Base weight for jump expert influence (optimized)")
    parser.add_argument("--jump_weight", type=float, default=8.28611953486444,
                        help="Weight multiplier for jump loss (optimized)")
    parser.add_argument("--decline_threshold", type=float, default=74.25427747155432,
                        help="Threshold for identifying potential decline cases (optimized)")
    parser.add_argument("--decline_expert_weight", type=float, default=0.85621601194762,
                        help="Base weight for decline expert influence (optimized)")
    parser.add_argument("--decline_weight", type=float, default=6.769917156070596,
                        help="Weight multiplier for decline loss (optimized)")
    parser.add_argument("--pde_expert_weight", type=float, default=0.5394894580093814,
                        help="Base weight for PDE expert influence (optimized)")
    parser.add_argument("--pde_weight", type=float, default=5.172326697890094,
    ...
    parser.add_argument("--learning_rate", type=float, default=0.00023689089570144521,
                        help="Learning rate (optimized)")
    parser.add_argument("--weight_decay", type=float, default=6.323728543271069e-07,
