    ...
    def train(self, train_loader, val_loader=None, epochs=100, lr=0.0005, weight_decay=1e-6,
        ...
        for epoch in range(epochs):
            ...
            for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs}"):
                ...
                population_2006 = batch['population_2006'].to(self.device)
                coordinates = batch['coordinates'].to(self.device)
                targets = batch['target'].to(self.device)
                
                # Calculate population changes for each ethnicity
                population_change = targets - population_2006  # [batch_size, num_ethnicities]
                
                # Create case type masks for each ethnicity
                # Shapes will be [batch_size, num_ethnicities]
                is_colonization = (population_2006 <= self.colonization_threshold)
                is_decline = (~is_colonization) & (population_2006 <= self.decline_threshold) & (population_change <= 0)
                ...
                outputs, expert_weights, expert_classes = self.model(
                    features, population_2006, coordinates, 
    ...
    def get_predictions(self, data_loader):
        ...
        with torch.no_grad():
            for batch in data_loader:
                ...
                all_base_populations.append(population_2006.cpu().numpy())
        ...
        # Add base populations (2006) for each ethnicity
        for i, eth in enumerate(self.ethnicity_names):
            data_dict[f"{eth}_2006"] = all_base_populations[:, i]
        ...
        if len(all_targets) > 0:
            ...
            for i, eth in enumerate(self.ethnicity_names):
                eth_2006 = all_base_populations[:, i]
                eth_actual = all_targets[:, i]
                
                # Calculate all flags
                is_colonization = (eth_2006 <= self.colonization_threshold)
                is_decline = (~is_colonization) & (eth_2006 <= self.decline_threshold)
                
                pop_change = eth_actual - eth_2006
