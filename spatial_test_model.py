...
def main(args):
    ...
    if hasattr(test_loader.dataset, 'ethnicity_columns'):
        ...
        if 'test_pop_2006' in data:
            ethnicity_names = [col for col in data['test_pop_2006'].columns if col != 'DAUID']
    ...
    try:
        ...
        # Check if test data has ground truth (2016 population)
        has_ground_truth = any(f"{k}_2016" in data for k in ['test_pop', 'test_pop_2016'])
